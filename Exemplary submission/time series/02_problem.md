...
Handle missing values in crypto features (10-30%), crisis periods (15%), weekends (20%), and macro event times (40%) using imputation methods that do not introduce forward-looking bias.
...
Handle extreme outliers appropriately — particularly flash crash returns (-20% to -50%) and gas price spikes — using robust methods rather than naive removal.
...
Use a temporal (chronological) train-validation split where validation data occurs strictly after training data — not random shuffling.
...
Do not use future information including lookahead features, contemporaneous target values, or features computed using data from timestamps at or after the prediction time.
...
Account for the 5 distinct market regimes (normal 70%, volatile 22%, crisis 5%, flash crash 1%, weekend 2%) through regime-specific features or regime-weighted approaches.
...
Incorporate institutional regulatory features (lcr_ratio, nsfr_ratio, funding_pressure) as they directly determine required liquidity buffers under Basel III.
...
Leverage timezone-dependent lead-lag relationships: FX leads crypto during London/NY overlap (13:00-16:00 UTC), crypto leads FX during Asian session (00:00-08:00 UTC).
...
Incorporate trading session structure (Asia, Europe, US) and session overlap periods which exhibit distinct U-shaped intraday liquidity patterns.
...
Do not randomly shuffle time-series data during training, which would destroy temporal dependencies and regime continuity.
...
Apply stationarity-inducing transformations such as differencing or log-returns to handle non-stationary time-series with regime shifts.
...
Recognize and handle 5-60 minute block missing patterns (0.2% probability) that represent system outages, using methods respecting temporal causality.
