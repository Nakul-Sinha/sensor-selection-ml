...
TOTAL_BUDGET_S = float(os.environ.get("ERIS_BUDGET_S", 4200))
SEED = 20240808
...
MARGIN_THRESHOLDS = (0.75, 1.25, 1.85)
UTIL_MEAN_W = 0.18
UTIL_RED_W = 0.035
...
def plan_utilities(R: np.ndarray) -> np.ndarray:
    ...
    sq = pair_sqdiff(R)
    D = np.sqrt(sq[:, PLANS, :].mean(axis=2))
    C = abs_corr(R)
...
def main() -> None:
    ...
    cat_train = M_train[:, PAIRS[:, 0], PAIRS[:, 1]]
    ...
    yflat = P_train.reshape(n_train * N_TILES)
